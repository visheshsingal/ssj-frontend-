        ...
        const authCheck = async () => {
            try {
                const res = await axios.get(
                    `${import.meta.env.VITE_SERVER_URL}/api/v1/auth/delivery-agent-auth`,
                    {
                        headers: {
                ...
                if (error.response?.status === 401 && !isContextLoading) {
                    ...
                    setTimeout(() => {
                        toast.error("Delivery Agent Privileges Required!", {
                            toastId: "userNotDeliveryAgent",
                        });
